...
export function useMentorDashboardData(userId: string): MentorDashboardData {
  ...
  const todaysGroups = (() => {
    ...
    return groups.filter((g) => (g as any)?.next_practical_at && isSameDay(new Date((g as any).next_practical_at), today));
  })();

  // Fetch QR codes for first 3 groups
  const qrGroups = groups ? groups.slice(0, 3) : [];
  ...
  const primaryGroup = groups && groups.length > 0 ? groups[0] : null;
  ...
  const isLoading = profileLoading || skillsLoading || groupsLoading || attendanceLoading || qrQueries.some(q => q.isLoading);
  
  // Combine errors
  const error = profileError || skillsError || groupsError || attendanceError || qrQueries.find(q => q.error)?.error;
  ...
  return {
    ...
    groups: groups || [],
    todaysGroups,
    qrCodes: qrQueries.map(q => q.data).filter(Boolean) as any,
    attendanceReport: attendanceReport || null,
    isLoading,
