...
export default function Page() {
  ...
  return (
    <Protected>
      ...
      <FadeInFromBottom>
        <div
          className={` w-full min-h-[100vh] lg:overflow-hidden px-2 lg:px-0 lg:pl-[20%] pb-5 py-2 lg:pr-4`}
        >
          ...
          <div className=" w-full flex flex-col lg:flex-row items-center justify-between">
            <div className=" w-full lg:w-[60%]">
              <h1 className=" font-bold text-[#379E37] text-xl mb-4">
                Course Code
              </h1>
              ...
              <div
                className={`${montserrat.className} text-sm font-thin w-full bg-white h-[250px] overflow-scroll rounded-md py-3 mb-4`}
              >
                {courses?.map((course, idx) => {
                  return (
                    // @ts-ignore
                    <h1
                      onClick={() => updateCourseCode(course)}
                      key={`${course.course}-${idx}`}
                      className="cursor-pointer w-full bg-white py-2 px-10 border-b border-b-slate-300"
                    >
                      {course.course}
                    </h1>
                  );
              ...
              >
                {specializations.map((specialization, idx) => {
                  return (
                    <h1
                      onClick={() => updateCourseSpecialization(specialization)}
                      key={`${specialization}-${idx}`}
                      className="cursor-pointer w-full bg-white py-2 px-10 border-b border-b-slate-300 uppercase"
                    >
                      {specialization}
                    </h1>
                  );
      ...
      <div className=" w-full text-center mb-4">
        ...
        <div className=" w-[80%] m-auto py-4 bg-[#98c898] rounded-lg mb-4">
          ...
          <div className=" text-center mb-4">
            <p>Course Group</p>
            <h1 className=" uppercase font-extrabold text-3xl">
              {selectionDetails.course_specialization}
            </h1>
          </div>
        ...
        {isCourseToastShown && (
          <h1 className=" text-red-500 mb-4">Please select a course !</h1>
        )}
        <div className=" w-full flex flex-col items-center justify-center">
          <button
            onClick={handlePaymentClick}
            type="button"
            className={"w-[80%] bg-[#379E37] text-white mb-4 py-2 rounded-md"}
            disabled={loading}
          >
            {loading ? "Loading..." : "Make Payment"}
          </button>
          <Link href={"/payment"} className=" text-sm text-[#379E37] underline">
